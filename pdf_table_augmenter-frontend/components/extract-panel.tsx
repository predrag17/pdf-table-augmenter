...
interface ExtractPanelProps {
  file: File;
  onClose: () => void;
  onTableResult: (tables: any[]) => void;
  onImageResult: (images: any[]) => void;
  onFormulaResult: (formulas: any[]) => void;
}
...
export default function ExtractPanel({
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      if (data.length > 0) {
        if (extractOption === "tables") onTableResult(data);
        else if (extractOption === "images") onImageResult(data);
        else if (extractOption === "formulas") onFormulaResult(data);
        onClose();
      } else {
